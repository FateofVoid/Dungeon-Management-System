...
test("continued scouting generates a complete city report inside the neighboring Sector", () => {
  const dms = atTierOne();
  ...
  dms.story.scouting.perimeterActive = true; dms.story.scouting.perimeterCycles = 1;
  dms.dungeon.tier = 2; DMS.applyDerivedState(dms); DMS.updateQuests(dms, { notify: false });
  const city = dms.story.localCity;
  assert.equal(city.name, "", "the city is not generated by Tier entry alone");
  assert.equal(dms.quests.records["story-beyond-perimeter-1"].status, "active");
  dms.story.scouting.perimeterCycles = 2;
  const briefing = DMS.execute(dms, "city briefing");
  assert.ok(city.name);
  assert.ok(city.appearance);
  assert.ok(city.sectorId);
  assert.equal(dms.world.lustria.sectors[city.sectorId].cityId, city.id);
  assert.match(briefing, new RegExp(city.name));
  assert.match(briefing, /Appearance:/);
