...
| Rooms and cohorts | Implemented foundation | Construction, tasks, Tier, Expansion, assignments, Worker and Soldier cohorts, production, Worker disruption, real cohort housing, private suites, Named Residences, Current Stay, detainment assignments, and aggregate modifiers are tracked. There is no individual Worker state. |
| Player-facing availability | Implemented foundation | Tier and Story progression gate visibility; current construction and shop options expose generated identity, requirements, readiness, in-progress/completed state, and target-specific purchases. Menus, cards, natural actions, and mutations share the same legality checks. |
...
| Dungeon Signature | Implemented display foundation | A deterministic Nascent/Awakened state, Strength, Theme, and broad detectability flag appear in status, Plot Essentials, and recovery-equivalent state. Authentication, emission sources, detection contests, suppression, and political consequences remain unimplemented. |
